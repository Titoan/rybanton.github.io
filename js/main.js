...
let slideBtnRight = document.querySelector('.go-next');
slideBtnRight.addEventListener('click', slideRight);

function slideRight(){
	let activeElement = document.querySelector('.slide-img.active');	

	let nextElement = activeElement.nextElementSibling;		

	let mainImgCollection = nextElement.parentElement.children;
		mainImgCollection = Array.prototype.slice.call(mainImgCollection);
		
	const index = mainImgCollection.indexOf(nextElement);
	let coureselImg = document.querySelectorAll('.img-container');
	console.log(coureselImg);

	if(index <= mainImgCollection.length -1){
		activeElement.classList.remove('active');
		nextElement.classList.add('active');
		coureselImg[index].style.marginBottom = "25px"		
	}
}


let slideBtnLeft = document.querySelector('.go-prev');
...
function slideLeft(){	
	...
	let mainImgCollection = activeElement.parentElement.children;
		mainImgCollection = Array.prototype.slice.call(mainImgCollection);

	let coureselImg = document.querySelectorAll('.img-container');

		
	const index = mainImgCollection.indexOf(activeElement);
	if(index != 0){
		activeElement.classList.remove('active');
		prevElement.classList.add('active');
		coureselImg[index].style.marginBottom = "0"
	}
}